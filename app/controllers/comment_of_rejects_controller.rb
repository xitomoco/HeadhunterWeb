class CommentOfRejectsController < ApplicationController
  ...
  def create
    ...
    if @comment_of_reject.save
      @job_application = JobApplication.where({vacuancy_job_id: "#{@vacuancy_job.id}", user_id: "#{@user.id}"})
  ...
  def params_comment_of_reject
    params.require(:comment_of_reject).permit(:vacuancy_job_id, :user_id, :comment)
  end
end